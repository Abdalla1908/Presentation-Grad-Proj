const PARTS = [
    { path: './Assets/Device/p1-1.stl', color: 0x1a1a2e, label: 'Screen', texture: './Assets/Device/screen.png' },
    { path: './Assets/Device/p1-2.stl', color: 0x1a1a2e, label: 'Screen' },
    { path: './Assets/Device/p2-1.stl', color: 0x101010, label: 'Device Front' },
    { path: './Assets/Device/p2-2.stl', color: 0x101010, label: 'Device Back' },
    { path: './Assets/Device/p2-3.stl', color: 0x101010, label: 'Device Cover' },
    { path: './Assets/Device/p3-1.stl', color: 0x151515, label: 'Camera Mount' },
    { path: './Assets/Device/p3-2.stl', color: 0x000000, label: 'TPU Feet' },
    { path: './Assets/Device/p4-1.stl', color: 0x303030, label: 'Speaker Case Back' },
    { path: './Assets/Device/p4-2.stl', color: 0xa0a0a0, label: 'Speaker' },
    { path: './Assets/Device/p4-3.stl', color: 0x303030, label: 'Speaker Case Front' },
    { path: './Assets/Device/p5.stl',   color: 0x1a1a1a, label: 'Camera' },
    { path: './Assets/Device/p6.stl',   color: 0x104010, label: 'Raspberry Pi 5' },
    { path: './Assets/Device/p7.stl',   color: 0xdb7500, label: 'Hailo-8L AI HAT' },
];

const CONFIG = {
    euler:       new THREE.Euler(-1.55, 0, -1, 'XYZ'),
    scale:       1.5,
    ambLight:    0.5,
    keyLight:    1.5,
    damping:     0.85,
...
const _c = (...pts) => new THREE.CatmullRomCurve3(pts);

// Each part has moves: { duration (real seconds), to: Vector3 offset from home }
// syncWith: index  — mirror another part's spline parameter exactly
// until: N         — mirror the leader's first N moves, then continue with own moves
// Parts with syncWith share the leader's computed sp value — perfect sync, zero drift.

const EXPLODE_STAGES = [
    // [0] Screen GROUP: Forward, Stops First, Take A Small Shift Down
    { moves: [ { duration: 0.6, to: _v(0, -0.25, -0.02) }, { duration: 0.6, to: _v(0, -0.20, -0.04) } ]},

    // [1] Device Front: Forward, Stops After Screen
    { moves: [ { duration: 0.6, to: _v(0, -0.3, 0) }, { duration: 0.6, to: _v(0, -0.5, 0) } ]},
    
    // [2] Device Back: FIXED
    { moves: [] },
    
    // [3] Device Cover: Backward (Down), Then Up
    { moves: [ { duration: 0.6, to: _v(0, 0.25, -0.1) }, { duration: 0.9, to: _v(0, 0.6, 0.05) } ]},
    
    // [4] Camera Mount: moves forward with group, stops after device front
    { syncWith: 1, until: 2, moves: [ { duration: 0.6, to: _v(0, -0.6, 0.05) } ]},
    
    // [5] TPU Feet: Down, Backward (Down)
    { moves: [ { duration: 0.6, to: _v(0, 0, -0.05) }, { duration: 0.6, to: _v(0, 0.4,  -0.1) } ]},
    
    // [6] Speaker Case Back: leader
    { moves: [ { duration: 0.6, to: _v(0, 0.125, 0.15) }, { duration: 0.9, to: _v(0, 0.15, -0.075) } ]},

    // [7] Speaker: mirrors 6 entirely
    { syncWith: 6, until: 2, moves: [ { duration: 0.6, to: _v(-0.3, 0.15, -0.075) } ] },

    // [8] Speaker Case Front: follows 6 for first 2 moves, then opens left
    { syncWith: 6, until: 2, moves: [ { duration: 0.6, to: _v(-0.35, 0.15, -0.075) } ]},

    // [9] Camera: syncs with Camera Mount for first 2 moves, then arcs up
...
(function prepareStages() {
    ...
    // Returns the resolved spline + cumulative for a stage, building it if needed.
    // Handles chains: a syncWith stage whose leader is also a syncWith stage.
    function resolveStage(stage) {
        ...
        if (stage.syncWith !== undefined) {
            ...
            resolveStage(leader); // ensure leader is ready first
            ...
            if (!stage.moves || stage.moves.length === 0) {
                ...
                // Partial follower: mirror leader for `until` moves, then own path
...
export function renderDevice(cell) {
    ...
    function makeSlider(min, max, value, step, onChange, decimals = 2) {
        ...
        val.textContent = parseFloat(value).toFixed(decimals);

        s.oninput = () => {
            val.textContent = parseFloat(s.value).toFixed(decimals);
    ...
    canvas.style.cssText = 'position: absolute; left: 120px; right: 0; top: 0; bottom: 0; width:calc(100% - 240px); height:100%;';
    ...
    .then((meshes) => {
        ...
        (function animate() {
            ...
            // Clock advances at exactly 1 real second per second toward the target
            ...
            meshList.forEach((m, i) => {
                ...
                // Walk sync chain to find the actual spline + param to sample
                function sampleStage(st) {
                    if (st.syncWith !== undefined) {
                        const leader = st._leader;

                        if (!st._ownSpline) {
                            // Pure mirror — delegate entirely to leader
                        ...
                        if (clock <= leaderHandoff.end) {
                            // Still in shared phase — delegate to leader
                            return sampleStage(leader);
                        }
                    ...
                    if (!st._spline) return; // FIXED — stays at home
    ...
    function buildPanel() {
        ...
        const explodeBtn = makeBtn('<i class="fas fa-expand-arrows-alt"></i>', () => setExplode(explodeT < 0.5 ? 1 : 0));
        ...
        const vExplodeBtn = makeBtn('<i class="fas fa-expand-arrows-alt"></i>', () => setExplode(explodeT < 0.5 ? 1 : 0));
        ...
        const expSlWrap = document.createElement('div');
        expSlWrap.style.display = 'none';
        const expSl = makeSlider(0, 1, 0, 0.01, (v) => { explodeTarget = v; });
        sliders.explode = expSl;
        expSlWrap.appendChild(expSl.el);
        panel.appendChild(expSlWrap);
        ...
        const scaleSl = makeSlider(0.3, 3.0, CONFIG.scale, 0.01, (v) => { scaleTarget = v; });
        sliders.scale = scaleSl; panel.appendChild(scaleSl.el);

        panel.appendChild(makeDivider());
        panel.appendChild(makeLabel('Spinning'));
        const spinSl = makeSlider(-0.05, 0.05, 0, 0.01, (v) => { spinSpeed = v; }, 2);
        sliders.spin = spinSl; panel.appendChild(spinSl.el);

        panel.appendChild(makeDivider());
        panel.appendChild(makeLabel('Ambient Light'));
        ...
        const keySl = makeSlider(0, 5, CONFIG.keyLight, 0.01, (v) => { keyTarget = v; });
